...
## Load nCounter matrix (maybe not )
read_ncounter_matrix <- function(file = NULL, sep = "\t") {
  ### Header
  nc_header <- colnames(data.table::fread(file = file, sep = sep, nrows = 0, header = TRUE))
...
(min(length(sig.genes) * 10, 5000) + 300)
    ...
    # gsea_go <- 
    if (any(unlist(msigdb.do), kegg.do, do.do, go.do, wp.do, reactome.do, mesh.do, custom.do) & (length(which(deg.idx)) >= enr.min.genes | gsea.force) & !is.null(species)) {
