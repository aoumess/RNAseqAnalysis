...
"${ENVTOOL}" create --prefix "${INSTALL_ENV_PREFIX}" -c conda-forge -c bioconda -c nodefaults -c r --channel-priority 0 -f "${ENVRECIPE}"

## Activate the environment
"${ENVTOOL}" activate "${INSTALL_ENV_PREFIX}"

## Install glmmSeq (and update 2 CRAN dependencies: abind & glmmTMB) & kmndirs packages (from github)
